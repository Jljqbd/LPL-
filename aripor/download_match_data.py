...
class download_match_data():
    ...
    def download(self):
        ...
        if json_data['data']['red']['teamid']==json_data['data']['info']['winner']:
            ...
            win_color = 'blue'
            fail_color = 'red'
        winner_index = self.atd.teamsname.index(winner_name) # 胜利者队伍编号
        fail_index = self.atd.teamsname.index(fail_name) # 失败者队伍编号
        
        self.atd.team_win_fail_num[0][winner_index] += 1 #成功队伍胜场+1
        self.atd.team_win_fail_num[1][fail_index] += 1 #失败队伍败场+1

        copy_team_Hc = self.atd.Hc[winner_index] # 作为统计后的对比镜像，使用英雄的
        copy_team_Jc = self.atd.Jc[winner_index] # 作为统计后的对比镜像，各个位置的
        ...
        for i in range(len(json_data['data']['eventLine'])): # 那个时间段的
            for j in range(len(json_data['data']['eventLine'][i]['list'])): #这个时间段的这一个事件
                ...
                if event_data[j]['color'] == 'red': # 红蓝双方坐标统一转化为蓝方坐标
                ...
                if event_data[j]['type']!='CHAMPION_KILL' or eventername ==' ': #如果不是击杀事件就跳过不统计
                    if eventername!=' ': #如果事件有队伍名称的话
                       self.atd.team_event[self.atd.teamsname.index(eventername.split('.')[0])] += 1 # 队伍事件数+1 
                    continue
                index_1 = self.atd.teamsname.index(eventername.split('.')[0]) # 这一事件主体队伍第一维度
                ...
                if event_data[j]['killerId'] in hero_index_blue.keys():
                    ...
                    if event_data[j]['killerId'] not in self.atd.remove_hero:#如果这个英雄不是缺省英雄的话那么也+1
        ...
        if self.atd.win_Hc[winner_index].shape[0] !=0 and self.atd.win_Hc[winner_index].shape[1] !=0:
            ...
            # 老数据复制到新矩阵中
            new_win_Hc_body[:old_win_Hc_shape[0], :old_win_Hc_shape[1]] = self.atd.win_Hc[winner_index] # 在更新win_Hc之前，存老win_Hc
        ...
        if winner_index != len(self.atd.win_Hc) - 1: # 不能让winner访问下标溢出
